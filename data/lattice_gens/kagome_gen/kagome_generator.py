...
def make_system(n: int, m: int, k: int, offset: float = 1.48387382444563, dilute: float = 0, mute=False) -> np.ndarray:
    ...
    x = np.array([1, 0.5, 1.5, 1, 0])
    y = np.array([4*np.sqrt(3)/6, np.sqrt(3)/6, np.sqrt(3)/6, 2*np.sqrt(3)/6, 0])
    z = np.array([0, 0, 0, offset/2, offset/2])

    # Directions of the spin in the unit cell
    mx = np.array([0, -cos30, cos30, 0, 0])
    my = np.array([1, -sin30, -sin30, 0, 0])
    mz = np.array([0, 0, 0, 1, 1])
    ...
    # Create the full system by stacking the layers
    system = np.empty((0, 6))
    for i in range(k):
        system = np.vstack((system, surf + [0, 0, offset*i, 0, 0, 0]))
